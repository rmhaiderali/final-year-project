  ...
  const jobListing = (
    <div className="grid md:grid-cols-2 gap-6">
      {currentJobs?.map((job, jobIndex) => {
        const applicants = job.applicants.map(
          (applicant) => applicant.documentId
        )

        const acceptedApplicants = job.acceptedApplicants.map(
          (applicant) => applicant.documentId
        )

        const rejectedApplicants = job.rejectedApplicants.map(
          (applicant) => applicant.documentId
        )
        ...
        return (
          <Card
            key={job.documentId + currentUserStatus}
          ...
          >
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center text-muted-foreground">
                    ...
                    <span>Salary: {job.salary} PKR</span>
                  </div>
                  <div className="flex items-center text-muted-foreground">
            ...
            <CardFooter className="flex gap-2">
              {!user?.isCompany && currentUserStatus === "notapplied" && (
                <Button
                  ...
                  onClick={async (e) => {
                    ...
                    jobs[indexOfFirstJob + jobIndex] = response.data
                    setJobs([...jobs])
                  }}
                  disabled={isExpired}
                >
              ...
              <Link href={"/job/" + job.documentId} className="w-full">
                <Button variant="outline" className="w-full">
                  View Details
                </Button>
              </Link>
          ...
          <div className="grid lg:grid-cols-4 gap-6">
            ...
            <div className="lg:col-span-3 space-y-6">
              {areJobsLoading ? (
                ...
                <>
                  {/* Job Statistics */}
                  <JobStatistics {...getJobStatistics(jobs)} />
